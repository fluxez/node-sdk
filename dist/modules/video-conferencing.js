...
class VideoConferencingClient {
    ...
    async getRecording(roomId) {
        try {
            ...
            const response = await this.httpClient.get(`/video-conferencing/rooms/${roomId}/recording`);
            return response.data.data;
        }
        catch (error) {
     ...
     * ```
     */
    async downloadRecording(egressId) {
        try {
            this.logger.debug('Downloading recording', { egressId });
            const response = await this.httpClient.get(`/video-conferencing/recordings/${egressId}/download`, {
