...
export class SchemaClient {
  ...
  async registerSchema(request: RegisterSchemaRequest): Promise<RegisterSchemaResponse> {
    try {
      ...
      const response = await this.http.post<ApiResponse<RegisterSchemaResponse>>(
        '/schema/register',
        request
      );
   ...
   */
  async getSchema(schemaIdentifier: string): Promise<SchemaDefinition> {
    try {
      this.logger.debug('Getting schema', { schemaIdentifier });

      const response = await this.http.get<ApiResponse<SchemaDefinition>>(
        `/schema/${encodeURIComponent(schemaIdentifier)}`
      );
   ...
   */
  async migrateSchema(request: MigrateSchemaRequest): Promise<MigrateSchemaResponse> {
    try {
      ...
      const response = await this.http.post<ApiResponse<MigrateSchemaResponse>>(
        '/schema/migrate',
        request
      );
   ...
   */
  async configureTable(request: ConfigureTableRequest): Promise<ConfigureTableResponse> {
    try {
      ...
      const response = await this.http.post<ApiResponse<ConfigureTableResponse>>(
        `/table/${encodeURIComponent(request.config.tableName)}/config`,
        request
      );
   ...
   */
  async describeTable(tableName: string): Promise<TableStructure> {
    try {
      this.logger.debug('Describing table', { tableName });

      const response = await this.http.get<ApiResponse<TableStructure>>(
        `/table/${encodeURIComponent(tableName)}/describe`
      );
   ...
   */
  async listTables(options: ListTablesOptions = {}): Promise<ListTablesResponse> {
    try {
      ...
      if (options.includeViews !== undefined) params.append('includeViews', String(options.includeViews));
      if (options.includeSize !== undefined) params.append('includeSize', String(options.includeSize));
      if (options.includeStats !== undefined) params.append('includeStats', String(options.includeStats));

      const response = await this.http.get<ApiResponse<ListTablesResponse>>(
  ...
  async createIndex(request: CreateIndexRequest): Promise<CreateIndexResponse> {
    try {
      ...
      const response = await this.http.post<ApiResponse<CreateIndexResponse>>(
        `/table/${encodeURIComponent(request.tableName)}/index`,
        request
      );
   ...
   */
  async dropTable(request: DropTableRequest): Promise<DropTableResponse> {
    try {
      ...
      const response = await this.http.delete<ApiResponse<DropTableResponse>>(
        `/table/${encodeURIComponent(request.tableName)}`,
        { data: request }
      );
  ...
  async getTableConfig(tableName: string): Promise<TableConfig> {
    try {
      ...
      const response = await this.http.get<ApiResponse<TableConfig>>(
        `/table/${encodeURIComponent(tableName)}/config`
      );
   ...
   * @returns Configuration response
   */
  async updateTableConfig(tableName: string, config: Partial<TableConfig>): Promise<ConfigureTableResponse> {
    try {
      this.logger.debug('Updating table config', { tableName, config });

      const request: ConfigureTableRequest = {
        config: { tableName, ...config }
      };

      const response = await this.http.put<ApiResponse<ConfigureTableResponse>>(
        `/table/${encodeURIComponent(tableName)}/config`,
        request
      );
  ...
  async dropIndex(tableName: string, indexName: string): Promise<{ success: boolean; message: string }> {
    try {
      ...
      const response = await this.http.delete<ApiResponse<{ success: boolean; message: string }>>(
        `/table/${encodeURIComponent(tableName)}/index/${encodeURIComponent(indexName)}`
      );
  ...
  async getTableIndexes(tableName: string): Promise<IndexDefinition[]> {
    try {
      ...
      const response = await this.http.get<ApiResponse<IndexDefinition[]>>(
        `/table/${encodeURIComponent(tableName)}/indexes`
      );
   ...
   */
  async createTable(config: any): Promise<any> {
    try {
   ...
   */
  async validateSchema(schema: SchemaDefinition): Promise<{ valid: boolean; errors: string[]; warnings: string[] }> {
    try {
      this.logger.debug('Validating schema', { schemaName: schema.name });

      const response = await this.http.post<ApiResponse<{ valid: boolean; errors: string[]; warnings: string[] }>>(
        '/schema/validate',
        { schema }
      );
   ...
   */
  async migrate(schema: Record<string, any>, options?: {
    dryRun?: boolean;
    force?: boolean;
    dropTables?: boolean;
    dev?: boolean;
  ...
  async syncSchema(schema: Record<string, any>, options?: {
    ...
    try {
      ...
      const response = await this.http.post<ApiResponse<any>>('/schema/sync', {
        schema,
        dryRun: options?.dryRun
      });
  ...
  private createSchemaError(originalError: any, code: string, tableName?: string, schemaName?: string): SchemaError {
    const error = new Error(originalError.message || 'Schema operation failed') as SchemaError;
