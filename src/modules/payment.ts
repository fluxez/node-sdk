...
export class PaymentClient {
   ...
   * ```
   */
  async updateConfig(updates: UpdatePaymentConfigRequest): Promise<PaymentConfig> {
    try {
  ...
  async addPriceId(
    ...
    try {
      ...
      const response = await this.httpClient.post<ApiResponse<PriceIdConfig>>(
        `/tenant-payment/${organizationId}/${projectId}/prices`,
        { priceId }
      );
  ...
  async removePriceId(
    ...
    try {
      ...
      await this.httpClient.delete(
        `/tenant-payment/${organizationId}/${projectId}/prices/${priceId}`
      );
  ...
  async getActiveSubscriptionByMetadata(
    ...
    try {
      ...
      this.logger.debug(`[SDK] Base URL: ${(this.httpClient as any).config?.baseURL || 'unknown'}`);
